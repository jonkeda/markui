...
| `v`  | Vertical list corner                        |
| `>`  | Horizontal list corner                      |
| `w`  | Wrapped list corner                         |
...
- Open-right boxes must NOT have a trailing `+`
...
`[...]` is a button labeled `...`. It no longer has special list-truncation semantics.
...
`v` corners define vertical list items:
...
`+--@Name--+` — a typed container with a component type for codegen.
