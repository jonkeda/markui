...
| 5 | `[...]` | List truncation | Exactly `...` inside `[]` |
| 6 | `[- N +]` | Stepper | `[-` ... `+]` with number inside |
| 7 | `[===..]` | Slider/Progress | Only `=` and `.` inside `[]` |
| 8 | `[***.]` | Rating | Only `*` and `.` inside `[]`, 3+ chars |
| 9 | `[[content]]` | Active tab/page | Double brackets |
| 10 | `[...][v]` or `[...][^]` | Split button | Two `[]` touching, second is `[v]`/`[^]` |
| 11 | `[text v]` or `[text ^]` | Accordion/Expander | Trailing ` v` or ` ^` before `]` |
| 12 | `[<]` `[>]` | Prev/Next button | Single `<` or `>` |
| 13 | `[#N text]` | Icon button | `#` + digit inside `[]` |
| 14 | `[text]` | Button | Default for `[]` — label text inside |
| 15 | `<@name>` | Custom input | First char after `<` is `@` |
| 16 | `<text v>` | Dropdown closed | Trailing ` v` before `>` |
| 17 | `<text ^>` | Dropdown open | Trailing ` ^` before `>` |
| 18 | `<___>` | Text input | Contains underscores |
| 19 | `<text>` | Text input (value) | Angle brackets, no special chars |
| 20 | `{[on]/off}` | Toggle (on) | `[` marks active, `/` separates |
| 21 | `{on/[off]}` | Toggle (off) | `[` marks active, `/` separates |
...
`*` corners signal a repeatable card. `+` corners signal a structural box. The parser checks the corner character to set the node type.
