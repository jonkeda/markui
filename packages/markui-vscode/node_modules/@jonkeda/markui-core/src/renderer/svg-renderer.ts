import { WidgetNode, ThemeColors } from '../types';

export function renderToSvg(tree: WidgetNode, theme: ThemeColors): string {
  ...
  parts.push(
    ...
    `<defs><style>`,
    `.mu-text{font-family:${esc(theme.fontFamily)};font-size:${theme.fontSize}px;fill:${theme.foreground}}`,
    `.mu-heading{font-family:${esc(theme.fontFamily)};fill:${theme.headingColor};font-weight:700}`,
    `.mu-link{font-family:${esc(theme.fontFamily)};font-size:${theme.fontSize}px;fill:${theme.linkColor};text-decoration:underline}`,
    `</style></defs>`,
...
function renderBox(
  node: WidgetNode, t: ThemeColors, out: string[],
  baseX: number, baseY: number, cw: number, lh: number, isCard: boolean
): void {
  const x = baseX + node.col * cw;
  const y = baseY + node.row * lh;
  ...
  if (node.level && node.level > 0) {
    out.push(
      `<line x1="${x}" y1="${y + lh * 0.5}" x2="${x + w}" y2="${y + lh * 0.5}" ` +
      `stroke="${t.border}" stroke-width="1" opacity="0.7"/>`
    );
    if (node.text) {
      out.push(
        `<text x="${x + 8}" y="${y + lh * 0.7}" class="mu-text" font-weight="600" font-size="${t.fontSize * 0.95}px">${esc(node.text)}</text>`
      );
    }
  ...
  if (node.text) {
    out.push(
      `<text x="${x + 8}" y="${y + lh * 0.7}" class="mu-text" font-weight="600">${esc(node.text)}</text>`
    );
  }
...
function renderButton(
  ...
  out.push(
    ...
    `fill="${t.buttonBg}" stroke="${t.buttonBorder}" stroke-width="1"/>`,
...
function renderIconButton(
  ...
  const h = lh + 4;
  out.push(
    `<rect x="${x}" y="${y + 2}" width="${w}" height="${h}" rx="4" fill="${t.buttonBg}" stroke="${t.buttonBorder}"/>`,
    `<text x="${x + 6}" y="${y + h / 2 + 6}" fill="${t.buttonFg}" font-family="${esc(t.fontFamily)}" font-size="10px">#${node.iconIndex ?? 0}</text>`,
    `<text x="${x + 20}" y="${y + h / 2 + 6}" fill="${t.buttonFg}" font-family="${esc(t.fontFamily)}" font-size="${t.fontSize}px">${esc(node.text ?? '')}</text>`
  );
}
...
function renderSplitButton(
  ...
  const by = y + 2;
  out.push(
    `<rect x="${x}" y="${by}" width="${textW + arrowW}" height="${h}" rx="4" fill="${t.buttonBg}" stroke="${t.buttonBorder}"/>`,
    `<line x1="${x + textW}" y1="${by}" x2="${x + textW}" y2="${by + h}" stroke="${t.buttonBorder}"/>`,
    `<text x="${x + textW / 2}" y="${by + h / 2 + 4}" text-anchor="middle" fill="${t.buttonFg}" font-family="${esc(t.fontFamily)}" font-size="${t.fontSize}px">${esc(node.text ?? '')}</text>`,
    `<text x="${x + textW + arrowW / 2}" y="${by + h / 2 + 4}" text-anchor="middle" fill="${t.buttonFg}" font-family="${esc(t.fontFamily)}" font-size="10px">\u25BC</text>`
  );
}

function renderNavButton(
  node: WidgetNode, t: ThemeColors, out: string[],
  x: number, y: number, cw: number, lh: number, arrow: string
): void {
  const s = lh;
  out.push(
    `<rect x="${x}" y="${y + 2}" width="${s}" height="${s}" rx="3" fill="${t.buttonBg}" stroke="${t.buttonBorder}"/>`,
    `<text x="${x + s / 2}" y="${y + 2 + s / 2 + 4}" text-anchor="middle" fill="${t.buttonFg}" font-family="${esc(t.fontFamily)}" font-size="10px">${arrow}</text>`
  );
}
...
function renderLink(
  ...
  out.push(
    `<text x="${x}" y="${y + lh * 0.7}" class="mu-link">${esc(node.text ?? '')}</text>`
  );
}
...
function renderTab(
  ...
  out.push(
    `<rect x="${x}" y="${y + 1}" width="${w}" height="${h}" rx="4 4 0 0" ` +
    `fill="${active ? t.activeTabBg : t.inactiveTabBg}" stroke="${t.border}" stroke-width="1"/>`
  );
  out.push(
    `<text x="${x + 8}" y="${y + h / 2 + 5}" fill="${active ? t.activeTabFg : t.inactiveTabFg}" ` +
